...
public class LifeSafetyZoneObjectTest {
    ...
    public void intrinsicReporting() throws Exception {
        ...
        assertEquals(EventState.normal, lsz.getProperty(PropertyIdentifier.eventState)); // Still normal at this point.
        clock.plus(600, TimeUnit.MILLISECONDS, 600, TimeUnit.MILLISECONDS, 0, 80);
        assertEquals(EventState.lifeSafetyAlarm, lsz.getProperty(PropertyIdentifier.eventState));
        assertEquals(new StatusFlags(true, false, false, false), lsz.getProperty(PropertyIdentifier.statusFlags));
        ...
        assertEquals(((BACnetArray<TimeStamp>) lsz.getProperty(PropertyIdentifier.eventTimeStamps))
        ...
        assertEquals(EventState.lifeSafetyAlarm, lsz.getProperty(PropertyIdentifier.eventState)); // Still lifeSafetyAlarm at this point.
        clock.plus(600, TimeUnit.MILLISECONDS, 600, TimeUnit.MILLISECONDS, 0, 40);
        assertEquals(EventState.normal, lsz.getProperty(PropertyIdentifier.eventState));
        assertEquals(new StatusFlags(false, false, false, false), lsz.getProperty(PropertyIdentifier.statusFlags));
    ...
    public void changeOfLifeSafetyAlgoA1() throws Exception {
        ...
        assertEquals(EventState.offnormal, lsz.getProperty(PropertyIdentifier.eventState));
        assertEquals(1, listener.notifs.size());
        ...
        assertEquals(EventState.offnormal, lsz.getProperty(PropertyIdentifier.eventState)); // Still offnormal at this point.
    ...
    public void intrinsicReportingWithFault() throws Exception {
        ...
        Thread.sleep(40);
        assertEquals(EventState.fault, lsz.getProperty(PropertyIdentifier.eventState));
        assertEquals(new StatusFlags(true, true, false, false), lsz.getProperty(PropertyIdentifier.statusFlags));
        assertEquals(1, listener.notifs.size());
        Map<String, Object> notif = listener.notifs.remove(0);
        assertEquals(new UnsignedInteger(10), notif.get("processIdentifier"));
        assertEquals(rd1.getObjectIdentifier(), notif.get("initiatingDevice"));
    ...
    public void covNotifications() throws Exception {
        ...
        d2.send(rd1, new SubscribeCOVRequest(new UnsignedInteger(987), lsz.getId(), new Boolean(false),
